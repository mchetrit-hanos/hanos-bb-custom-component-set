(() => ({
  ...
  jsx: (() => {
    ...
    const { TableCell, TableSortLabel, Tooltip } = window.MaterialUI.Core;
    const {
      horizontalAlignment,
      headerText,
      property,
    ...
    return visible ? (
      ...
      <TableCell
        ...
        onClick={() => {
          if (handleRowClick)
            handleRowClick(myEndpoint, context);
        }
        }
      >
        {isEmpty ?
          <Tooltip title={(headerOnly) ? '' : columnText} placement='bottom-start'>
