(() => ({
  ...
  jsx: (() => {
    ...
    const {
      ...
      autoLoadTakeAmount,
      multiSelectRows,
      selectedRowCountLocationInText,
    ...
    const handleSelectAllClick = (event) => {
      ...
      if (event.target.checked && !event.target.indeterminate) {
        const newSelectedRows = results.map((n) => n.id);
                  ...
                  {multiSelectRows ? (
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        onClick={(event) => handleCheckboxClick(event, value.id)}
                      />
                    </TableCell>
                  ) : <></>}
                  {children}
    ...
    useEffect(() => {
      if (selectedCount !== selectedRows.length) {
        setSelectedCount(selectedRows.length);
        B.triggerEvent('onSelectedRowChange', selectedRows.toString());
        B.triggerEvent('onSelectedRowCountChange', (selectedRowCountLocationInText === 'start') ? `${selectedRows.length.toString()} ${selectedRowCountText}` : `${selectedRowCountText} ${selectedRows.length.toString()}`);
      }
    }, [selectedRows]);
                ...
                <TableRow classes={{ root: classes.headerRow }}>
                  {multiSelectRows ? (
                    <TableCell padding="checkbox">
                      <Checkbox
                        classes={{ root: classes.checkbox }}
                        indeterminate={selectedCount > 0 && selectedCount < results.length}
  ...
  styles: B => theme => {
    ...
    return {
      ...
      checkbox: {
        '&.MuiCheckbox-root': {
          color: ({ options: { checkboxColor } }) => style.getColor(checkboxColor),
        },
        '&.Mui-checked': {
          color: ({ options: { checkboxCheckedColor } }) => `${style.getColor(checkboxCheckedColor)} !important`,
          '&:hover': {
            backgroundColor: ({ options: { checkboxCheckedColor } }) => [
              getOpacColor(style.getColor(checkboxCheckedColor), 0.04),
              '!important',
            ],
          },
        },
      ...
      bodyRow: {
        '&:nth-of-type(odd)': {
          '& td span': {
            color: ({ options: { alternateRowColors, oddColor } }) =>
              alternateRowColors ? `${style.getColor(oddColor)}` : '',
          },
          backgroundColor: ({ options: { alternateRowColors, oddBackgroundColor } }) =>
            alternateRowColors && [style.getColor(oddBackgroundColor), '!important'],
        },
        '&:nth-of-type(even)': {
          '& td span': {
            color: ({ options: { alternateRowColors, evenColor } }) =>
              alternateRowColors ? `${style.getColor(evenColor)} !important` : '',
          },
          backgroundColor: ({ options: { alternateRowColors, evenBackgroundColor } }) =>
            alternateRowColors && [style.getColor(evenBackgroundColor), '!important'],
        },
