(() => ({
  ...
  jsx: (() => {
    ...
    const handleValidation = () => {
      if (!error) {
        if (required) {
          setErrorState((required) ? required && !isValidDate(selectedDate) : !isValidDate(selectedDate));
        }
        else {
          setErrorState((selectedDate) ? !isValidDate(selectedDate) : false);
        }
      }
    };
    ...
    const DateTimeCmp = (
      <DateTimeComponent
        ...
        required={required}
        autoOk={(variant === 'inline') ? closeDialogOnSelect : false}
        disabled={disabled}
