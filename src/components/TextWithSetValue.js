(() => ({
    ...
    jsx: (() => {
        ...
        useEffect(() => {
            if (isDev) {
                GetLinkedContent(parsedContent);
            }
        }, [isDev, parsedContent]);

        useEffect(() => {
            GetLinkedContent(useText(content));
        }, []);

        B.defineFunction('SetValue', value => {
            GetLinkedContent(value);
        });
